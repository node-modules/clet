...
class TestRunner extends BaseSDK {
  ...
  stdin(expected, respond) {
    ...
    this._addChain(async () => {
      // check stdout
      const result = this.ctx.result;
      await pWaitFor(() => {
        const stdout = result.stdout.substring(this._lastPromptIndex);
        if (utils.validate(stdout, expected)) return true;
        if (result.stopped) throw new Error('wait for prompt, but proccess is terminate.');
        return false;
