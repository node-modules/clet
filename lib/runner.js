...
class TestRunner extends EventEmitter {
  ...
  // Perform the test, optional, when you await and `end()` could be omit.
    ...
    this.middlewares.unshift(async function cleanup(ctx, next) {
      try {
        await next();
        ctx.logger.success('Test pass.\n');
        // ensure it will return context
        return ctx;
      } catch (err) {
        ctx.logger.success('Test failed.\n');
        throw err;
      } finally {
  ...
  async _execCommand() {
    ...
    this.proc.then(res => {
      ...
      if (res.failed && !res.isCanceled && !res.killed) {
        this.logger.warn('Command failed with: %s', res.originalMessage || res.shortMessage);
      }
    });
  ...
  stdin(expected, respond) {
    ...
    this._addChain(async function stdin(ctx) {
      ...
      for (const str of respond) {
        ...
        // auto add \n
        this.proc.stdin.write(str.replace(/\r?\n$/, '') + EOL);
   ...
   * @param {Boolean} [opts.clean = true] - whether rm dir after test
   * @return {TestRunner} instance for chain
   */
  cwd(dir, opts = {}) {
    ...
    if (init) {
      this.use(async function initCwd(ctx, next) {
        ...
        assert(cmdType === 'fork' && !utils.isParent(dir, cmd), `rm ${dir} is too dangerous`);
