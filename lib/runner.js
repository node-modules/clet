...
class TestRunner extends BaseSDK {
  ...
  async _execCommand() {
    ...
    this.proc.then(res => {
      ...
      if (res.failed) this.logger.info('Command failed with: %s', res.originalMessage);
    });
  ...
  stdin(expected, respond) {
    ...
    this._addChain(async ({ result, res }) => {
      // check stdout
      await pWaitFor(() => {
        assert(!result.stopped, `wait for prompt, but proccess is terminate with error: ${res && res.originalMessage}`);
