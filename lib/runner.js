...
class TestRunner extends BaseSDK {
  ...
  _addChain(fn, position) {
    ...
    const chains = this._chains[position];
    assert(chains, `unexpected position ${position}`);
    chains.push(fn);
    return this;
  }
   ...
   * run a chain
   *
   * @param {'before'|'running'|'after'|'end'} position - which chain to run
   * @private
   */
  async _runChain(position) {
    const chains = this._chains[ position ];
    assert(chains, `unexpected position ${position}`);
    for (const fn of chains) {
      this.logger.info('run %s chain fn:', position, fn.name);
    ...
    this.middlewares.unshift(async (ctx, next) => {
      try {
        ...
        if (ctx.proc && !ctx.result.stopped) {
          this.logger.warn(`still alive, kill ${ctx.proc.pid}`);
          ctx.proc.kill();
          await ctx.proc;
        }
      }
  ...
  async _execCommand() {
    ...
    assert(cmd, 'cmd is required');
    assert(utils.existsSync(cmdOpts.cwd), `cwd ${cmdOpts.cwd} is not exists`);
    ...
    this.proc.then(res => {
      ...
      if (res.failed) {
        this.logger.info('Command failed with: %s', res.originalMessage || res.shortMessage);
      }
    });
  ...
  stdin(expected, respond) {
    ...
    this._addChain(async function stdin(ctx) {
      // check stdout
      const isPrompt = utils.validate(ctx.result.stdout.substring(this._lastPromptIndex), expected);
      if (!isPrompt) {
        try {
          await pEvent(this, 'stdout', {
            rejectionEvents: [ 'close' ],
            filter: () => {
              return utils.validate(ctx.result.stdout.substring(this._lastPromptIndex), expected);
            },
          });
        } catch (err) {
          throw new Error(`wait for prompt, but proccess is terminate with error: ${ctx.res && (ctx.res.originalMessage || ctx.res.shortMessage)}`);
        }
      }

      // remember last index
      this._lastPromptIndex = ctx.result.stdout.length;
   ...
   */
  kill(signal, options) {
    return this._addChain(async function kill() {
